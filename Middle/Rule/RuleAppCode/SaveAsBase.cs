...
namespace Approve.Common
{
    ...
    public class SaveAsBase
    {
        ...
        /// <summary>
        /// 根据DataGrid导出Excel【UTF-8】
        /// </summary>
        /// <param name="DG_List"></param>
        ...
        public void SaveAsExc(DataGrid DG_List, string Title, System.Web.HttpResponse Response)
        {
            ...
            //			DG_List.DataSource = dt;
            //			DG_List.DataBind();
            for (int j = 0; j < DG_List.Items.Count; j++)
            {
                for (int i = 0; i < DG_List.Columns.Count; i++)
                {
                    DG_List.Items[j].Cells[i].Attributes.Add("style", "vnd.ms-excel.numberformat:@");
                }
            }
            DG_List.RenderControl(oHtmlTextWriter);
            //oStringWriter.Write(oHtmlTextWriter.ToString());
            string pattern = @"<a\s+[^>]+?>\s*(<font\s+[^>]+?>)?(?<company>\w+\b)";
            Regex reg = new Regex(pattern, RegexOptions.IgnoreCase);

            Response.Write(reg.Replace(oStringWriter.ToString(), "${company}"));
            oHtmlTextWriter.Close();
            oStringWriter.Close();
        ...
        /// <summary>
        /// 根据参数判断显示的编码规范[返回Default的编码格式]
        /// </summary>
        /// <param name="DG_List"></param>
        /// <param name="Title"></param>
        /// <param name="Response"></param>
        public void SaveAsExc(DataGrid DG_List, string Title, System.Web.HttpResponse Response, string codeStandard, int tag)
        {
            string sFileName = Title;
            string sHeadTittle = Title;
            Response.Clear();
            Response.Buffer = true;
            if (codeStandard.ToLower() == "gb2312")
            {
                Response.Charset = "gb2312";  //设置了类型为中文防止乱码的出现   
            }
            else if (codeStandard.ToLower() == "utf-8")
            {
                Response.Charset = "utf-8";  //设置了类型为中文防止乱码的出现   
            }
            else
        ...
        public void SaveAsDoc(System.Web.UI.Control control, string title, System.Web.HttpResponse Response)
        {
            ...
            Response.Buffer = true;
            Response.Charset = "utf-8";  //设置了类型为中文防止乱码的出现   
            Response.AppendHeader("Content-Disposition", "attachment;filename=" + HttpUtility.UrlEncode(title, System.Text.Encoding.UTF8) + ".doc"); //定义输出文件和文件名 
            Response.ContentEncoding = System.Text.Encoding.GetEncoding("utf-8");//设置输出流为简体中文 
