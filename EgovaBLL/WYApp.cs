...
namespace EgovaBLL
{
    public class WYApp
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static bool FileInfo(string fappid, string FManageTypeId, string DFUserID)
        {
            ...
            switch (FManageTypeId)
            {
                ...
                case "14401":
                    try
                    {
                        IsZG(XMBH, FBaseInfoId, DFUserID, fappid);
                ...
                #region 项目失去申请-14402
                case "14402": //项目失去申请
                    str.Append("insert into wy_xm_jbxx_history([Fid],[XMBH],[XMMC],[XMSD],[XMDZ],[JSDW],[JSDWDZ],[XMLX],[XMZLX],[JSXZ],[JSMS],[XMZTZ],[JSGM],[JSNR],[BH],[JSDWZZJFDM],[JSDWFR],[JSDWFRDH],[JSDWJSFZR],[JSDWJSFZRZC],[JSDWJSFZRDH],[FUpDeptId],[FUpDeptName],[FTime],[FSystemId],[FCreateUser],[FCreateTime],[FIsDeleted],[FBaseInfoID],[StandardStatus],[SourceAppID],[InSystemLostReasonID],[InSystemLostRemarks],[JGQYID]) ");
                    str.Append("select newid(),'" + XMBH + "',[XMMC],[XMSD],[XMDZ],[JSDW],[JSDWDZ],[XMLX],[XMZLX],[JSXZ],[JSMS],[XMZTZ],[JSGM],[JSNR],[BH],[JSDWZZJFDM],[JSDWFR],[JSDWFRDH],[JSDWJSFZR],[JSDWJSFZRZC],[JSDWJSFZRDH],[FUpDeptId],[FUpDeptName],'" + DateTime.Now + "',[FSystemId],'" + DFUserID + "','" + DateTime.Now + "',0,[FBaseInfoID],[StandardStatus],[SourceAppID],1,'自行申请失去','' from wy_xm_jbxx where xmbh='" + XMBH + "'");
                    str.Append("Update WY_XM_JBXX set FBaseInfoID=0 where XMBH='" + XMBH + "'");
                    if (!rc.PExcute(str.ToString())) { return false; }
                    break;
                ...
                case "14406": //项目财务年报
                    ...
                    str.Append(IntoCWQK(FBaseInfoId, fappid, XMBH));
                    rc.PExcute(str.ToString());
                    break;
                #endregion
        ...
        public static bool OperateRYToBZ(string fAppId)
        {
            ...
            if (dtRY.Rows.Count > 0)
            {
                ...
                foreach (DataRow dr in dtRY.Rows)
                {
                    ...
                    //修正公共数据
                    string appid = sl["SourceAppID"].ToString();
                    ...
                    if (!rc_RYB.SaveEBase("CF_Empxy_PersonBaseInfo", sl, "FID", so))
                    {
                        return false;
                    }
                }
        ...
        {
            StringBuilder str = new StringBuilder();
            str.Append("Insert into WY_XM_HTBA(FID,XMBH,HTBH,XMHQFS,HTSXRQ,HTJZRQ,HTQDRQ,WTDW,JGRQ,FCreateUser,FCreateTime,FTime,ZStatus,FBaseInfoID,FSystemID,FIsDeleted,SourceAppID,FLinkID) ");
            str.Append("select NewID(),'" + XMBH + "',HTBH,XMHQFS,HTSXRQ,HTJZRQ,HTQDRQ,WTDW,JGRQ,FCreateUser,FCreateTime,'" + DateTime.Now + "',ZStatus,'" + fbaseinfoid + "',FSystemID,FIsDeleted,'" + fappid + "',FID from YW_WY_XM_HTBA where FAppID='" + fappid + "'");
            return str.ToString();
        }
        ...
        private static string IntoCWQK(string fbaseinfoid, string fappid, string XMBH)
        {
            StringBuilder str = new StringBuilder();
        ...
        {
            StringBuilder str = new StringBuilder();
            str.Append("Insert into WY_RY_JBXX(FID,XMBH,fEntName,fzzjgdm,fPersonName,fCardID,fSex,fBirthday,fMZ,fAddress,fPosition,fbysj,fbyzsh,fMajor,flxdz,fSchool,fyzbm,fdzyx,fbgdh,fCardType,fgrdh,fphoto,fState,fTechnical,fzcqdsj,fzczsh,fxl,fxw,fxwzsh,fMemo,fFifteenCardID,fSourceID,fBaseInfoID,FSystemID,FCreateUser,FCreateTime,FTime,SourceAppID,FIsDeleted,FLinkID) ");
            str.Append("select NewID(),'" + XMBH + "',fEntName,fzzjgdm,fPersonName,fCardID,fSex,fBirthday,fMZ,fAddress,fPosition,fbysj,fbyzsh,fMajor,flxdz,fSchool,fyzbm,fdzyx,fbgdh,fCardType,fgrdh,fphoto,fState,fTechnical,fzcqdsj,fzczsh,fxl,fxw,fxwzsh,fMemo,fFifteenCardID,fSourceID,'" + fbaseinfoid + "',FSystemID,FCreateUser,FCreateTime,'" + DateTime.Now + "','" + fappid + "',FIsDeleted,FID from YW_WY_RY_JBXX where FAppID='" + fappid + "'");
            return str.ToString();
        }
        ...
        private static bool IsZG(string XMBH, string FBaseInfoId, string OperateUser, string fappid)
        {
            StringBuilder str = new StringBuilder();
            string fbiid = rc.GetSignValue("select FBaseInfoId from WY_XM_JBXX where XMBH='" + XMBH + "'");
            if (!string.IsNullOrEmpty(fbiid))
            {
                str.Append("insert into wy_xm_jbxx_history([Fid],[XMBH],[XMMC],[XMSD],[XMDZ],[JSDW],[JSDWDZ],[XMLX],[XMZLX],[JSXZ],[JSMS],[XMZTZ],[JSGM],[JSNR],[BH],[JSDWZZJFDM],[JSDWFR],[JSDWFRDH],[JSDWJSFZR],[JSDWJSFZRZC],[JSDWJSFZRDH],[FUpDeptId],[FUpDeptName],[FTime],[FSystemId],[FCreateUser],[FCreateTime],[FIsDeleted],[FBaseInfoID],[StandardStatus],[SourceAppID],[InSystemLostReasonID],[InSystemLostRemarks],[JGQYID]) ");
                str.Append("select newid(),'" + XMBH + "',[XMMC],[XMSD],[XMDZ],[JSDW],[JSDWDZ],[XMLX],[XMZLX],[JSXZ],[JSMS],[XMZTZ],[JSGM],[JSNR],[BH],[JSDWZZJFDM],[JSDWFR],[JSDWFRDH],[JSDWJSFZR],[JSDWJSFZRZC],[JSDWJSFZRDH],[FUpDeptId],[FUpDeptName],'" + DateTime.Now + "',[FSystemId],'" + OperateUser + "','" + DateTime.Now + "',0,[FBaseInfoID],[StandardStatus],'" + fappid + "',3,'企业申请在管强制失去','" + FBaseInfoId + "' from wy_xm_jbxx where xmbh='" + XMBH + "'");
                
                str.Append(";Delete WY_XM_JBXX where XMBH='" + XMBH + "'");
                str.Append(";Delete WY_XM_KZXX where XMBH='" + XMBH + "'");
                str.Append(";Delete WY_XM_HTBA where XMBH='" + XMBH + "'");
                str.Append(";Delete WY_XM_YZWYHBA where XMBH='" + XMBH + "'");
                str.Append(";Delete WY_XM_YZWYHCY where XMBH='" + XMBH + "'");
                str.Append(";Delete WY_RY_JBXX where XMBH='" + XMBH + "'");
                str.Append(";Delete WY_XM_XMQTXX where XMBH='" + XMBH + "'");
                rc.PExcute(str.ToString(), true);
                return true;
            }
            return false;
