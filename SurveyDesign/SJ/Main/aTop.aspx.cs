...
public partial class Admin_main_aTop : System.Web.UI.Page
{
    ...
    private void showInfo()
    {
        ...
        li_UserType.Text = " 用户类型：" + db.CF_Sys_SystemName.Where(t => t.FNumber.ToString() == CurrentEntUser.SystemId).Select(t => t.FDesc).FirstOrDefault().Replace("资质办理", "");


        lDate.Text = "今天是：" + string.Format("{0:yyyy年MM月dd日}", DateTime.Now) + " " + DateTime.Now.DayOfWeek.ToString();
        lLockInfo.Text += "" + "<a href='../../helpDoc/四川省勘察设计科技管理信息平台-设计单位用户操作手册.pdf' target='_blank' >操作说明下载</a>";
