...
public partial class Admin_main_aTop : System.Web.UI.Page
{
    ...
    private void showInfo()
    {
        ...
        string strTemp = db.CF_Sys_SystemName.Where(t => t.FNumber.ToString() == CurrentEntUser.SystemId).Select(t => t.FDesc).FirstOrDefault();
        if (!string.IsNullOrEmpty(strTemp))
            strTemp = strTemp.Replace("资质办理", "");
        li_UserType.Text = " 用户类型：" + strTemp;

        lDate.Text = "今天是：" + string.Format("{0:yyyy年MM月dd日}", DateTime.Now) + " " + DateTime.Now.DayOfWeek.ToString();
        lLockInfo.Text += "" + "<a href='../../helpDoc/四川省勘察设计科技管理信息平台-勘察单位用户操作手册.pdf' target='_blank' >操作说明下载</a>";
