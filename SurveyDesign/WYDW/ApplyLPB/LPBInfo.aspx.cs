...
public partial class WYDW_ApplyLPB_LPBInfo : WYPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (Type == "NEW")
            {
                ...
                string strSql_Floor = "select distinct RHC=convert(int,RHC) from dbo.YW_WY_XM_FWXX where Buildid='" +
                                      buildid + "' order by RHC desc";

                DataTable dt_Floor = rc.GetTable(strSql_Floor);

                string strsql_Unit =
                    "select t.dy as name,count(a.houseid) as PrefloorRoomCount  from( select distinct DY,buildid  from dbo.YW_WY_XM_FWXX ) as t inner join dbo.YW_WY_XM_FWXX as a on t.dy=a.dy ";
                strsql_Unit += "where t.buildid='" + buildid + "' and a.buildid='" + buildid + "' GROUP BY t.dy ";

                DataTable dt_Unit = rc.GetTable(strsql_Unit);


                string strSql_House = "select *  from dbo.YW_WY_XM_FWXX where Buildid='" + buildid +
                                      "' order by FH desc";
            ...
            {
                string buildid = Request.QueryString["BuildID"];//Convert.ToString(Session["WY_LPB_Buildid"]);

                string strSql_Floor = "select distinct RHC=convert(int,RHC) from dbo.WY_XM_FWXX where Buildid='" +
                                      buildid + "' order by RHC desc";

                DataTable dt_Floor = rc.GetTable(strSql_Floor);

                string strsql_Unit =
                    "select t.dy as name,count(a.houseid) as PrefloorRoomCount  from( select distinct DY,buildid  from dbo.WY_XM_FWXX ) as t inner join dbo.WY_XM_FWXX as a on t.dy=a.dy ";
                strsql_Unit += "where t.buildid='" + buildid + "' and a.buildid='" + buildid + "' GROUP BY t.dy ";

                DataTable dt_Unit = rc.GetTable(strsql_Unit);


                string strSql_House = "select *  from dbo.WY_XM_FWXX where Buildid='" + buildid +
                                      "' order by FH desc";
