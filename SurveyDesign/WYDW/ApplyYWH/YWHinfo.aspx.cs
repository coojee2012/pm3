...
public partial class WYDW_ApplyYWH_YWHinfo : WYPage
{
    ...
    private void saveData()
    {
        ...
        else
        {
            ...
            sl.Add("XMBH",Session["XMBH"].ToString());
    ...
    protected void YWHCY_List_ItemDataBound(object sender, DataGridItemEventArgs e)
    {
        if (e.Item.ItemIndex > -1)
        {
            ...
            e.Item.Cells[2].Text = "<a href='javascript:void(0)' onclick=\"showYWHCYWindow('YWHCYInfo.aspx?e=2&fid=" + fid + "','720','350');\">" + e.Item.Cells[2].Text + "</a>";
            //性别:0表示女，1表示男
    ...
    protected void btnDel_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            {
                ClientScript.RegisterStartupScript(GetType(), "", "<script>alert('请至少选择一条数据进行删除!')</script>");
            }
        }
        catch
        {
    ...
    private object getSelectedFids(bool IsSql, string tableName)
    {
        ...
        for (int i = 0; i < RowCount; i++)
        {
            ...
            if (cbx.Checked)
            {
                FId = ywhcy_List.Items[i].Cells[ColumnsCount - 2].Text.Trim();
