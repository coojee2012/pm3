...
public partial class JSDW_ApplyXZYJS_AuditMain_FirstAcceptJieJian : System.Web.UI.Page
{
    ...
    {
        string sql = string.Format("select A.ID,A.[FILE_NAME],COUNT(B.ID) TOTAL,A.Number FROM YW_FILE A LEFT JOIN (select * from YW_FILE_DETAIL) B ON A.ID=B.[FileId] WHERE A.YWBM='{0}'  GROUP BY A.[FILE_NAME],a.ID,A.Number", YWBM);
        DataTable table = rc.GetTable(sql);
    ...
    protected void btnAccept_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            //{
            string sql = "EXEC SP_GD_XZYJS @XMBH,@BH";
            SqlParameter[] param = new SqlParameter[2];
            param[0] = new SqlParameter() { ParameterName = "@XMBH", Value = YJS_ID, SqlDbType = SqlDbType.VarChar };//编号
            param[1] = new SqlParameter() { ParameterName = "@BH", Value = hfXMBH.Value, SqlDbType = SqlDbType.VarChar }; //项目编码
            rc.PExcute(sql, param);//办结归档
            // }

            sql = "EXEC JKC_PRO_DZ_insertXZ @ywbm";//打证
    ...
    private void SaveXZYJS()
    {
        if (TypeId == "5")
        {
            string sql = "Update YW_XZYJS set XZYJSBH=@XZYJSBH,HFJG=@HFJG,HFRQ=@HFRQ where ID=@ID";
            SqlParameter[] prams = new SqlParameter[4];
            prams[0] = new SqlParameter() { ParameterName = "@XZYJSBH", Value = txtXZYJSBH.Text, SqlDbType = SqlDbType.VarChar };
            ...
            prams[3] = new SqlParameter() { ParameterName = "@ID", Value = YJS_ID, SqlDbType = SqlDbType.VarChar };
