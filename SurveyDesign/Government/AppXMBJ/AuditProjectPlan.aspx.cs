...
public partial class JSDW_ApplyGCGH_AuditMain_AuditProjectPlan : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {
                string sql = @"select top 1 * from [YW_XMBJ] where YWBM='" + YWBM + "'";
                ...
                if (table != null && table.Rows.Count > 0)
                {
                    ...
                    hfBH.Value = row["XMBH"].ToString();
                    bindAuditList(row["YWBM"].ToString());
                    bindAuditOne();
                    ltrTZSText.Text = string.Format("<a href=\"../../ReportPrint/PrintList.aspx?TypeId=808001&YWBM={0}\" target=\"_blank\" id=\"SLDY\" style=\"display:none\"></a><a href=\"../../ReportPrint/PrintList.aspx?TypeId=808002&YWBM={0}\" target=\"_blank\" id=\"NOSLDY\" style=\"display:none\"></a>", row["ID"]);
                }
            }
        }
    }
    ...
    {
        string fileSql = string.Format("select FileDetailId,IsHave,Remark from YW_FILE_REMARK where YWBM='{0}'", YWBM);
    ...
    protected void btnAccept_Click(object sender, EventArgs e)
    {
        ...
        if (TypeId == "5")
        {
            string sql = "EXEC SP_GD_XMBJ @XMBH,@BH";
