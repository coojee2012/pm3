...
public partial class JSDW_ApplyGCGH_AuditMain_AuditProjectPlan : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            if (TypeId == "1")
            {
                ...
                btnYes.Visible = true;
                btnNoYes.Visible = true;
                btnNoAccept.Visible = true;
            }
            else if (TypeId == "10")
            {
                btnAccept.Text = "上报审批";
            ...
            {
                string sql = @"select top 1 * from [YW_GCGH] where YWBM='" + YWBM + "'";
                ...
                if (table != null && table.Rows.Count > 0)
                {
                    ...
                    bindAuditOne();
                    ltrTZSText.Text = string.Format("<a href=\"../../ReportPrint/PrintList.aspx?TypeId=606001&YWBM={0}\" target=\"_blank\" id=\"SLDY\" style=\"display:none\"></a><a href=\"../../ReportPrint/PrintList.aspx?TypeId=606002&YWBM={0}\" target=\"_blank\" id=\"NOSLDY\" style=\"display:none\"></a>", row["ID"]);
                }
            }
        }
    }
    ...
    protected void btnAccept_Click(object sender, EventArgs e)
    {
        ...
        {
            SaveGCGH();
            string sql = "EXEC SP_GD_GCGH @XMBH,@BH";
            ...
            sql = "EXEC JKC_PRO_DZ_insertGCGH @ywbm";//打证
    ...
    private void SaveGCGH()
    {
        if (TypeId == "5")
        {
            string sql = "Update YW_GCGH set GCGHXKZBH=@GCGHXKZBH,HFJG=@HFJG,HFRQ=@HFRQ where YWBM=@YWBM";
            SqlParameter[] prams = new SqlParameter[4];
            prams[0] = new SqlParameter() { ParameterName = "@GCGHXKZBH", Value = txtGCGHXKZBH.Text, SqlDbType = SqlDbType.VarChar };
