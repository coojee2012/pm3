...
public partial class JSDW_ApplyGCGH_AuditMain_AuditProjectPlan : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {
                string sql = @"select top 1 * from [YW_JGYS] where YWBM='" + YWBM + "'";
                ...
                if (table != null && table.Rows.Count > 0)
                {
                    ...
                    bindAuditOne();
                    hfJGId.Value = row["ID"].ToString();
                    hfBH.Value = row["XMBM"].ToString();
    ...
    public void bindAuditOne()
    {
        ...
        else
        {
            ...
            string sSql = string.Format(@"select top 1 er.FID,a.FID as PFID from CF_App_ProcessInstance a,CF_App_ProcessRecord er
    ...
    private void SaveFileMessage()
    {
        ...
        if (!string.IsNullOrEmpty(file))
        {
            ...
            foreach (var item in files)
            {
                ...
                _builder.AppendFormat("insert into YW_FILE_REMARK(YWBM,FileDetailId,IsHave,Remark)values('{0}',{1},{2},'{3}');", YWBM, items[0], items[1], items[2].Replace("'",""));
            }
    ...
    protected void btnAccept_Click(object sender, EventArgs e)
    {
        ...
        {
            Text = "办结";
            string sql = "EXEC SP_GD_JGYS @XMBH,@BH";
            SqlParameter[] param = new SqlParameter[2];
            param[0] = new SqlParameter() { ParameterName = "@XMBH", Value =hfJGId.Value, SqlDbType = SqlDbType.VarChar };//编号
            param[1] = new SqlParameter() { ParameterName = "@BH", Value = hfBH.Value, SqlDbType = SqlDbType.VarChar }; //项目编码
            rc.PExcute(sql, param);//办结归档
            //System.IO.File.AppendAllText("C:\\yujiajun.log", hfJGId.Value + ":" + hfBH.Value, System.Text.Encoding.Default);
            sql = "EXEC JKC_PRO_DZ_insertJGYS @ywbm";//打证
            rc.PExcute(sql, new SqlParameter() { ParameterName = "@ywbm", Value = hfYWBM.Value, SqlDbType = SqlDbType.VarChar });
        }
        tool.showMessageAndRunFunction(Text + "成功", "window.returnValue='1';window.close();");
    }
