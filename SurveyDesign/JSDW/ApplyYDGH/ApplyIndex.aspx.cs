...
public partial class JSDW_ApplyYDGH_ApplyIndex : System.Web.UI.Page
{
    ...
    {
        string sql = @"select A.*,B.YDGHMC,B.ID,B.ProjectType,case B.ProjectType when 1 then '房屋建筑' when 2 then '市政基础' else '其它' end as TypeName FROM CF_App_List A,YW_YDGH B WHERE A.FID = B.YWBM AND B.FBaseInfoId='" + CurrentEntUser.EntId + "'";
    ...
    protected void DG_List_ItemCommand(object source, DataGridCommandEventArgs e)
    {
        if (e.Item.ItemIndex > -1)
        {
            ...
            {
                string fAppId = FId;
                string YD_Id = e.Item.Cells[e.Item.Cells.Count - 1].Text;
                ...
                if (projectType == "1")
                    ...
                    Response.Write("<script language='javascript'>parent.parent.document.location='../ApplyYDGHSZ/AppMain/aIndex.aspx" + param + "';</script>");
               // Response.Write("<script language='javascript'>parent.parent.document.location='AppMain/aIndex.aspx" + param + "';</script>");
            }
            ...
            else if (e.CommandName == "Del")
            {
                ...
                sb.Append("delete from YW_YDGH where YWBM='" + FId + "';");
