...
public partial class JSDW_ApplyYDGH_Report : System.Web.UI.Page
{
    ...
    public void showInfo()
    {
        ...
        if (!string.IsNullOrEmpty(guid))
        {
            ...
            if (!string.IsNullOrEmpty(YD_Id))
            {
                string sql = "select top 1 YDGHMC,XMSD,XMSDMC,SBBM,IsAudit from YW_YDGH where ID='" + YD_Id + "'";
                DataTable table = rc.GetTable(sql);
                if (table != null && table.Rows.Count > 0)
                {
                    DataRow row = table.Rows[0];
                    hfIsAudit.Value = row["IsAudit"].ToString();
                    t_YDGHMC.Text = row["YDGHMC"].ToString();
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        if (appList != null)
        {
            ...
            sl[0].Add("FEmpName", t_YDGHMC.Text.Trim());
            ...
            sb.AppendFormat("update YW_YDGH set SBBM='{0}',SBRQ='{2}' where ID='{1}';", ddlLevel.SelectedValue, YD_Id,DateTime.Now);
