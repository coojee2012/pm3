...
public partial class JSDW_ApplyYDGH_FileAdd : System.Web.UI.Page
{
    ...
    private void ShowInfo()
    {
        ...
        if (!string.IsNullOrEmpty(TypeId) && !string.IsNullOrEmpty(YD_Id))
        {
            string sql = string.Format("select * from YW_FILE_DETAIL where FileId={0} and YWBM='{1}'", TypeId, YD_Id);
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        if (!string.IsNullOrEmpty(hfUpadFile.Value))
        {
            ...
            {
                string[] fileMessages = item.Split('|');
                string downLoadPath = item.Split('|')[0];
                string fileSize = item.Split('|')[1];
                string fileName = Path.GetFileName(downLoadPath);
                sql += string.Format("INSERT INTO YW_FILE_DETAIL(ID,FileId,YWBM,[FILE_NAME],FILE_SIZE,DownLoadPath)VALUES(NEWID(),{0},'{1}','{2}','{3}','{4}')", TypeId, YD_Id, fileName, fileSize, downLoadPath);
            }
