...
public partial class JSDW_ApplyYDGH_Report : System.Web.UI.Page
{
    ...
    public void showInfo()
    {
        ...
        if (!string.IsNullOrEmpty(guid))
        {
            ...
            if (!string.IsNullOrEmpty(XM_Id))
            {
                string sql = "select top 1 XMBJMC,XMSD,XMSDMC,SBBM,IsAudit from YW_XMBJ where ID='" + XM_Id + "'";
                DataTable table = rc.GetTable(sql);
                if (table != null && table.Rows.Count > 0)
                {
                    DataRow row = table.Rows[0];
                    hfIsAudit.Value = row["IsAudit"].ToString();
                    t_YDGHMC.Text = row["XMBJMC"].ToString();
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        if (appList != null)
        {
            ...
            sb.AppendFormat("update YW_XMBJ set SBBM='{0}' where ID='{1}';", ddlLevel.SelectedValue, XM_Id);
