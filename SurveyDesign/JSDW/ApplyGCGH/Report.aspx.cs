...
public partial class JSDW_ApplyYDGH_Report : System.Web.UI.Page
{
    ...
    public void showInfo()
    {
        ...
        if (!string.IsNullOrEmpty(guid))
        {
            ...
            {
                string sql = "select top 1 GCGHMC,XMSD,XMSDMC,SBBM,IsAudit from YW_GCGH where ID='" + GC_Id + "'";
                DataTable table = rc.GetTable(sql);
                if (table != null && table.Rows.Count > 0)
                {
                    DataRow row = table.Rows[0];
                    hfIsAudit.Value = row["IsAudit"].ToString();
                    t_GCGHMC.Text = row["GCGHMC"].ToString();
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        if (appList != null)
        {
            ...
            sb.AppendFormat("update YW_GCGH set SBBM='{0}',SBRQ='{2}' where ID='{1}';", ddlLevel.SelectedValue, GC_Id,DateTime.Now);
