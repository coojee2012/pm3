...
public partial class JSDW_ApplyYDGH_FileAdd : System.Web.UI.Page
{
    ...
    private void ShowInfo()
    {
        ...
        if (!string.IsNullOrEmpty(FileId) && !string.IsNullOrEmpty(GC_Id))
        {
            string sql = string.Format("select * from YW_FILE_DETAIL where FileId={0} and YWBM='{1}'", FileId, GC_Id);
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        if (!string.IsNullOrEmpty(hfUpadFile.Value))
        {
            ...
            foreach (var item in items)
            {
                ...
                sql += string.Format("INSERT INTO YW_FILE_DETAIL(ID,FileId,YWBM,[FILE_NAME],FILE_SIZE,DownLoadPath)VALUES(NEWID(),{0},'{1}','{2}','{3}','{4}')", FileId, GC_Id, fileName, fileSize, downLoadPath);
            }
