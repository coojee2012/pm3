...
public partial class JSDW_ApplyYDGH_ApplyIndex : System.Web.UI.Page
{
    ...
    {
        string sql = @"select A.*,B.GCMC,B.ID,B.ProjectType,case B.ProjectType when 1 then '房屋建筑' when 2 then '市政基础' else '其它' end as TypeName FROM CF_App_List A,YW_JGYS B WHERE A.FID = B.YWBM AND B.FBaseInfoId='" + CurrentEntUser.EntId + "'";
        if (!string.IsNullOrEmpty(t_YWFname.Text.Trim()))
        {
            sql += " and B.GCMC like '%" + t_YWFname.Text.Trim() + "%' ";
        }
    ...
    protected void DG_List_ItemCommand(object source, DataGridCommandEventArgs e)
    {
        if (e.Item.ItemIndex > -1)
        {
            ...
            {
                string fAppId = FId;
                string JG_Id = e.Item.Cells[e.Item.Cells.Count - 1].Text;
                ...
                var param = "?JG_Id=" + JG_Id + "&fAppId=" + fAppId;
                ...
                if (projectType == "1")
                    ...
                    Response.Write("<script language='javascript'>parent.parent.document.location='../ApplyJGYSSZ/AppMain/aIndex.aspx" + param + "';</script>");
                else
                    Response.Write("<script language='javascript'>parent.parent.document.location='AppMain/aIndex.aspx" + param + "';</script>");
            }
            ...
            else if (e.CommandName == "Del")
            {
                ...
                sb.Append("delete from YW_JGYS where YWBM='" + FId + "';");
