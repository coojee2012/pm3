<%@ Page Language="C#" AutoEventWireup="true" CodeFile="EntListSelCerti.aspx.cs"
    Inherits="JSDW_appmain_EntListSelCerti" %>
...
<body>
    ...
    <table width="98%" align="center" class="m_title">
        <tr>
            <th colspan="5">
                <asp:Label ID="lTitle" runat="server" Text="勘察单位" ForeColor="Red"></asp:Label>企业列表
            </th>
        </tr>
        <tr>
    ...
    <asp:Repeater ID="dg_List" runat="server" OnItemDataBound="dg_List_ItemDataBound">
        ...
        <ItemTemplate>
            <asp:Repeater ID="subList" runat="server" OnItemCommand="subList_ItemCommand" OnItemDataBound="subList_ItemDataBound">
                <ItemTemplate>
                    <tr class="m_dg1_i">
                        <%# Container.ItemIndex==0?
                                                        "<td rowspan='" + Eval("count") + "'>" + Eval("index") + "</td><td rowspan='" + Eval("count") + "'>" +
                        Eval("FName")   +"</td>":""%>
                        <td>
                            <%# Eval("FCertiNo")%>
                        </td>
                        ...
                        <td>
                            <asp:LinkButton ID="btnSelect" CommandName="Sel" runat="server">选择</asp:LinkButton>
                             <asp:HiddenField ID="hfFBaseInfoId" Value='<%# Eval("FID") %>' runat="server" />
            <asp:HiddenField ID="hfCertiId" Value='<%# Eval("fCertiId") %>' runat="server" />
                        </td>
                    </tr>
                </ItemTemplate>
            </asp:Repeater>
            <asp:HiddenField ID="hfFID" Value='<%# Eval("FID") %>' runat="server" />
            <asp:HiddenField ID="hfCount" Value='<%# Eval("count") %>' runat="server" />
        </ItemTemplate>
        <FooterTemplate>
            </table>
        </FooterTemplate>
