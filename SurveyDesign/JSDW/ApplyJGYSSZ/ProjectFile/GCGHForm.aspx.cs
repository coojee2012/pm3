...
public partial class JSDW_ApplyJGYS_ProjectFile_GCGHForm : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            string sql = string.Format("select top 1* from XM_JGYS_TRANS where YWBM='{0}' and TypeId=2", Id);
            ...
            sql = string.Format("select top 1 * from XM_GCGH where YWBM='{0}'", Id);
            table.Clear();
            ...
            if (table != null && table.Rows.Count > 0)
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    txtJSXMMC.Text = row["XMMC"].ToString();
                    txtJSWZ.Text = row["GCDD"].ToString();
                    txtMJ.Text = row["JZMJ"].ToString();
                    txtKD.Text = row["GD"].ToString();
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        if (string.IsNullOrEmpty(hfId.Value))//新增
        {
            ...
            sql = @"INSERT INTO [dbCenter].[dbo].[XM_GCGH]
                        ([ID]
                        ,[YWBM]
                        ,[GCGHBM]
                        ,[JSDW]
                        ...
                        ,[FZJG]
                        ,[FZRQ])
                    VALUES(@ID,@YWBM,@GCGHBM,@JSDW,@JSXMMC,@JSWZ,@JSGM,@MJ,@KD,@QT,@QTGG,@GCGHXKZBH,@FZJG,@FZRQ)";
            ...
            list.Add(new SqlParameter() { ParameterName = "@GCGHBM", Value = Guid.NewGuid().ToString(), SqlDbType = SqlDbType.VarChar });
            list.Add(new SqlParameter() { ParameterName = "@JSDW", Value = txtJSDW.Text, SqlDbType = SqlDbType.VarChar });
            list.Add(new SqlParameter() { ParameterName = "@JSXMMC", Value = txtJSXMMC.Text, SqlDbType = SqlDbType.VarChar });
            list.Add(new SqlParameter() { ParameterName = "@JSWZ", Value = txtJSWZ.Text, SqlDbType = SqlDbType.VarChar });
            list.Add(new SqlParameter() { ParameterName = "@JSGM", Value = txtJSGM.Text, SqlDbType = SqlDbType.VarChar });
            list.Add(new SqlParameter() { ParameterName = "@MJ", Value = txtMJ.Text, SqlDbType = SqlDbType.VarChar });
            list.Add(new SqlParameter() { ParameterName = "@KD", Value = txtKD.Text, SqlDbType = SqlDbType.VarChar });
            ...
            list.Add(new SqlParameter() { ParameterName = "@GCGHXKZBH", Value = txtGCGHXKZBH.Text, SqlDbType = SqlDbType.VarChar });
            ...
            sql += string.Format(@"INSERT INTO [dbo].[XM_JGYS_TRANS]
                       ...
                       ,[IsTrans]
                       ,[LY])
                        values(NEWID(),'{0}','{1}','{2}','{3}');", Id, 2, ddlIsTrans.SelectedValue, txtLY.Text.Replace("'", string.Empty));
        }
        else
        { //编辑
            sql = @"UPDATE [dbCenter].[dbo].[XM_GCGH]
                        SET [JSDW] = @JSDW
                            ,[JSXMMC] = @JSXMMC
                            ,[JSWZ] = @JSWZ
                            ,[JSGM] = @JSGM
            ...
            sql += string.Format(@"UPDATE [dbo].[XM_JGYS_TRANS]
                                       SET [IsTrans] = {0}
                                          ,[LY] = '{1}'
                                     WHERE YWBM='{2}' and TypeId=2;", ddlIsTrans.SelectedValue, txtLY.Text.Replace("'", string.Empty), Id);
        }
        pageTool tool = new pageTool(this.Page, "txt");
        sql += GetFileSql(YWBM);
