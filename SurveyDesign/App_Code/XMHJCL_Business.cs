...
public class XMHJCL_Business
{
    ...
    private DataTable GetXzyjs(string xmbh)
    {
        ...
        string sql = @"select   JSDW,XMMC,XMDZ,JSGM   from  XM_BaseInfo.dbo.XM_XZYJS where xmbh = '" + xmbh + "'";
    ...
    private DataTable GetJsydgh(string xmbh)
    {
        ...
        string sql = @"select   JSDW,XMMC,XMDZ,JSGM   from  XM_BaseInfo.dbo.XM_JSYDGH where xmbh = '" + xmbh + "'";
        return prjdb.GetTable(sql);
    }


    public DataTable GataBaseData(string xmbh,Enum 环节材料Enum)
    {
