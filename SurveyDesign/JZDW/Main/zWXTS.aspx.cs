    ...
    private void showAppList()
    {
        ...
        var v = from t in db.CF_App_List
                ...
                select new
                {
                    ...
                    t.FState,
                    t.FManageTypeId,
                    FReportDate = t.FManageTypeId == 28003 ? t.FReportDate : t.FAppDate,
                    t.FUpDeptId,
                    t.FYear,
                    t.FAppDate,
                    //工程名称 
                    ...
                    t.FBaseinfoId,
                    t.FToBaseinfoId,
                    FCmd = t.FId + "," + t.FManageTypeId + "," + t.FState,
                    //查询勘察单位的受理情况
    ...
    protected void App_List_ItemDataBound(object sender, DataGridItemEventArgs e)
    {
        if (e.Item.ItemIndex > -1)
        {
            ...
            { //当前用户为业务发起者
                switch (FState)
                {
                    case "0"://未上报
                        ...
                        if (FManageTypeId == "28002")
                        {
                            s = "<font color='#444444'>正在安排</font>";
                            r = "<font color='#444444'>正在安排</font>";
                        }
                        else if (FManageTypeId == "28004")
                        {
                            s = "<font color='#444444'>正在见证</font>";
                            r = "<font color='#444444'>正在见证</font>";
                        }
                        btnOp.Text = "删除";
                    ...
                    case "6"://已办结
                        ...
                        if (FManageTypeId == "28002")
                        {
                            ...
                            r = "<font color='green'>已安排</font>";
                        }
                        else if (FManageTypeId == "28004")
                        {
                            s = "<font color='green'>见证完毕</font>";
                            r = "<font color='green'>见证完毕</font>";
                        }
                        btnOp.Text = "--";
                        btnOp.CommandName = "";
                        btnOp.Attributes["onclick"] = "return false;";
                ...
                && FManageTypeId != "412" && FManageTypeId != "422")
            {
                ...
                {
                    case "28001"://项目见证受理
                        sUrl = "showAddWindow(\"../ApplyKCXMWT/Report.aspx?FAppId=" + FID + "\",700,480);";
                        break;
                    case "28002"://见证人员安排
                        sUrl = "showAddWindow(\"../applykcxmwt/Staffing.aspx?FAppId=" + FID + "\",700,680);";
                        break;
                    case "28004"://见证人员意见
                        sUrl = "showAddWindow(\"../ApplyXMJZ/Report.aspx?FAppId=" + FID + "\",700,500);";
                        break;
                }

                ((HtmlAnchor)e.Item.FindControl("a_PrjName")).HRef = "javascript:showAddWindow('../ApplyKCXMWT/ApplyBaseInfo.aspx?FDataID=" + FLinkId + "',800,700);";

                ((LinkButton)e.Item.FindControl("btnItemSee")).Attributes["onclick"] = sUrl + "return false;";
            }
            else if (FManageTypeId == "412" || FManageTypeId == "422")
            {
                ...
                {//确认合同备案后自动创建
                    ((Literal)e.Item.FindControl("lit_TS")).Text += "<img src=\"../../image/tip.gif\" style=\"cursor:pointer\" title=\"确认合同备案后自动创建，可删除，删除后可恢复\"/>";
                    ...
                    if (app280 != null && app280.FCount > 1)
                    {
                        ...
                        if (v != null)
                        {
                            ...
                            txt += "href=\"javascript:showAddWindow('../../KcsjSgt/ApplyKCJSXSC/Report.aspx?FAppId=" + v.FId + "',700,680);\">";
                            txt += "查看审图机构意见</a>";
                            ((Literal)e.Item.FindControl("lit_Count")).Text = ("(" + app280.FCount + "次," + txt + ")");
                        }
                    }
                    ((HtmlAnchor)e.Item.FindControl("a_PrjName")).HRef = "javascript:showAddWindow('../ApplyKCXMWT/ApplyBaseInfo.aspx?FDataID=" + FPriItemId + "',800,700);";
                }
                else
                {
                    e.Item.Cells[1].Text = pName;
                }
    ...
    protected void App_List_ItemCommand(object source, DataGridCommandEventArgs e)
    {
        if (e.CommandName == "See") //打开业务
        {
            ...
            if (s.Length == 3)
            {
                ...
                if (FManageTypeId == "28002" || FManageTypeId == "28004")
                {
                    HiddenField hfFLinkId = e.Item.FindControl("hfFLinkId") as HiddenField;
                    if (hfFLinkId != null)
                    {
                        Session["FAppId"] = hfFLinkId.Value;
                    }
                }
                else
                {
