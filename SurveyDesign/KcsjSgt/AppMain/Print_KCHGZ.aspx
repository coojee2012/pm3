<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Print_KCHGZ.aspx.cs" Inherits="KcsjSgt_AppMain_Print_KCHGZ" %>
...
<head runat="server">
    ...
    <script type="text/javascript">
        function checkLSH(obj) {
            ...
            if (state) {
                return confirm('设置为已打印后，不可再修流水号,确认要保存设置吗?');
            }
            return true;
        }
    </script>
                    ...
                    <asp:GridView ID="DG_List" runat="server" CssClass="m_dg1" Width="100%" HorizontalAlign="Center"
                        ...
                        <Columns>
                            ...
                            <asp:TemplateField HeaderText="工程名称">
                                ...
                                <ItemTemplate>
                                    <a href='javascript:showAddWindow("../ApplyKCWJSCWTSL/ApplyBaseInfo.aspx?FDataID=<%#Eval("FLinkId") %>",800,700);'>
                                        <%#Eval("FPrjName")%>
                                    </a>
                            ...
                            <asp:TemplateField HeaderText="操作">
                                <ItemStyle Wrap="false" />
                                <ItemTemplate>
                                    <asp:LinkButton ID="btnSave" runat="server" Text="保存" CommandName="DoSave" CommandArgument='<%#Eval("FID") %>'></asp:LinkButton>|<a
                                        href="<%=ReportServer %>SC-KCWJSCHGS.cpt&FAppId=<%#Eval("FID") %>&FPrjId=<%#Eval("FPrjId") %>&PrjId=<%#Eval("FPrjId") %>"
                                        target="_blank"> 打印</a>
                                </ItemTemplate>
                            </asp:TemplateField>
                        </Columns>
